...
if ( $MobilePhoneStatus == 1 ) {
	...
	if ( ( strlen($_SESSION['captcha']) != 6 || strlen($captcha) != 6 ) && strlen($action) > 17 ) {
		...
		echo $error = "Fehlermeldung 2: Die Zeichen aus dem Bild stimmen nicht &uuml;berein. Alle Buchstaben werden gross geschrieben.";
		ScreenStep2( $MobilePhone , $error); 
		exit(0);		
		...
		if ( $NewMobilePhoneStatus == 1 ) {
			...
			echo $error = "Mobilfunknummer $NewMobilePhone ist ung&uuml;ltig!";
			ScreenStep2( $MobilePhone , $error); 
			exit(0);		
		...
		echo $error = "Fehlermeldung 3: Die Zeichen aus dem Bild stimmen nicht &uuml;berein. Alle Buchstaben werden gross geschrieben.";
		ScreenStep2( $MobilePhone , $error); 
		exit(0);		

	} else {

		echo $error = "No correct action parameter";
		ScreenStep2( $MobilePhone , $error); 
		exit(0);
