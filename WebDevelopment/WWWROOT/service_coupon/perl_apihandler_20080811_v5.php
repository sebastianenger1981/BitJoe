...
if ( $MobilePhoneStatus == 1 && $CouponCodeStatus == 1 && $bj_SEC_MD5Status == 1 && $bj_UP_MD5Status == 1 && strlen($couponcode) == 10 ) {
	...
	# if ( strcmp($couponcode , $gift_code) != 0 ) {
	 if ( $couponcode != $gift_code ) {
		echo "Der Gutschein ist nicht gueltig!<br>";
		# echo "'$couponcode' != '$gift_code'";
		exit(0);
	...
	$SqlQuery 		= "SELECT `hc_contingent_volume_success`,`hc_contingent_volume_overall`,`web_flatrate_validUntil`,`web_servicetype`,`hc_userType`,`hc_sec1_MD5`,`hc_sec2_MD5`,`hc_sec3_MD5`,`hc_sec4_MD5`,`hc_sec5_MD5` FROM `$TABLE1` WHERE `web_mobilephone` = '$MobilePhone' AND `web_up_MD5` = '$bj_UP_MD5' AND `hc_abuse` = '0' LIMIT 1;";
	...
	if ( $MySqlArrayCheck ) {
		...
		while( $sql_results = mysql_fetch_array($MySqlArrayCheck)) {
			...
			$hc_userType			= $sql_results["hc_userType"];
	...
	# wenn jemand einen gutschein benutzt rutscht er von usertyp =3 ( handy anmeldung user ) auf zu einem normalen free user mit $hc_userType = 1
	if ( $hc_userType == 3 ) {
	...
	$TABLE1			= BJPARIS_TABLE;
	$SqlUpdateBJPARIS 	= "UPDATE `$TABLE1` SET `web_servicetype` = \"$web_servicetype_new\", `hc_contingent_volume_success` = \"$user_success_search\", `hc_contingent_volume_overall` = \"$user_overall_search\", `web_flatrate_validUntil` = \"$user_flat_validUntil\", `hc_userType` = \"$hc_userType\" WHERE CONVERT( `$TABLE1`.`web_mobilephone` USING utf8 ) = \"$MobilePhone\" LIMIT 1;";
