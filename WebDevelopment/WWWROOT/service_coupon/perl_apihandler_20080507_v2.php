...
if ( $MobilePhoneStatus == 1 && $CouponCodeStatus == 1 && $bj_SEC_MD5Status == 1 && $bj_UP_MD5Status == 1 && strlen($couponcode) == 10 ) {
	...
	$SqlQuery 		= "SELECT `hc_contingent_volume_success`,`hc_contingent_volume_overall`,`web_flatrate_validUntil`,`web_servicetype`,`hc_sec1_MD5`,`hc_sec2_MD5`,`hc_sec3_MD5`,`hc_sec4_MD5`,`hc_sec5_MD5` FROM `$TABLE1` WHERE `web_mobilephone` = '$MobilePhone' AND ( `hc_up_MD5` = '$bj_UP_MD5' OR `web_up_MD5` = '$bj_UP_MD5' ) AND `hc_abuse` = '0' LIMIT 1;";
	...
	$TABLE4			= USED_TABLE;
	$SqlQueryUSED_TABLE	= "INSERT INTO `$TABLE4` ( `used_mobilephone`, `used_code`, `used_date`) VALUES ( '$MobilePhone', '$gift_code', '$CurrentAsciiDate' );";
