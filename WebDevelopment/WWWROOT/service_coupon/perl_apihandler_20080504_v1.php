...
if ( $MobilePhoneStatus == 1 && $CouponCodeStatus == 1 && $bj_SEC_MD5Status == 1 && $bj_UP_MD5Status == 1 && strlen($couponcode) == 10 ) {
	...
	$TABLE2			= COUPON_TABLE;
	echo $SqlQueryCOUPON_TABLE	= "SELECT * FROM `$TABLE2` WHERE `gift_code` = '$couponcode' AND `gift_isValid` = '1' AND `gift_how_often_valid` > '0' AND '$CurrentDate' <= `gift_code_validUntil` LIMIT 1;";
	...
	echo "code: '$couponcode' , '$gift_code'<br>";

	# Code schon aufgebraucht 
	if ( strcmp($couponcode , $gift_code) != 0 ) {
		echo "Der Gutschein ist nicht gültig!";	
		exit(0);
	...
	# echo "DEGUB: $web_servicetype | $gift_type<br>";
