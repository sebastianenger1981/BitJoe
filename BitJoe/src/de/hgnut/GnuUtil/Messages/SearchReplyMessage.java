...
public class SearchReplyMessage extends Message {
	 ...
	 */
	SearchReplyMessage(short[] rawMessage, GnuConnection originatingConnection) {
	...
	public void buildPayload() {
		...
		try {
			...
			// client identifier
			short[] clientID = GUID.getClientIdentifier();
		...
		if (0 == fileRecords.size()) {
			...
			for (int i = 0; i < getFileCount(); i++) {
				...
				for (int z = 0; z < stringBytes.length; z++) {
					stringBytes[z] = ((Byte)stringData.elementAt(z)).byteValue();
				}
				
				if (sha1Present) {
					for (int z = 0; z < sha1Bytes.length; z++) {
						sha1Bytes[z] = ((Byte)sha1Data.elementAt(z)).byteValue();
					}
