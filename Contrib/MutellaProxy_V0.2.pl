...
while (!$DONE) {
  ...
  if ( $BlockerFlag == -1 ) {
	printf "[Connect from DNS $HandyClientHostName <-> IP $HandyClientIPAdress] - Rejected by IPBlocker\n";
...
sub FindFunction(){
	...
	$IO->WriteFile( "$FILE/$ClientID", $IOReadFromClient[5]);	# merke die den suchbegriff für die HandyID $ClientID
...
sub ResultFunction(){
	...
	if ( $QueryNumber eq 'NORESULTS' ) {
		...
		my $SendString = $CACHE->readCache( 0, 9, $SEARCH );

		if ( length($SendString) != 0 ) {
			print "DEBUG-QUERY-INVALID: $SendString\n";
	...
	if ( $NumberOfResults == 0 || $NumberOfResults == -1 ) {
		...
		$CACHE->writeCache( $SortedResultsArrayRef, $QueryClientID, $SEARCH );
