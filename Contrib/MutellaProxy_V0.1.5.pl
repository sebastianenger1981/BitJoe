...
#	$HandyClientSocket->blocking(0);
	...
	# Nach jeweils 5 Verbindungsanfragen installiere eine aktuelle UltraPeerListe
	if ( $ConnectionCounter	== 5 ) {
		$MUTELLA->GetAndInstallUltraPeerHostList();
		$ConnectionCounter = 0;
	};
...
sub FindFunction(){
	...
	my $ClientID = $CHECKSUM->MD5ToHEX( $hostinfo->name . time() . $MUTELLA->random() ); 
...
sub ResultFunction(){
	...
	if ( $#{$SortedResultsArrayRef} == 0 ) {
		...
		$CACHE->writeCache( $SortedResultsArrayRef, $QueryClientID );
