...
# perl -MCPAN -e 'install "WWW::Mechanizer"' 
...
use constant CRLF => "\r\n";
use constant PIDFILE => "/root/.mutella/PID.file";
...
print STDOUT "[Server $0 accepting clients on port 3381]\n";
...
while (!$DONE) {
  ...
  if ( $BlockerFlag == -1 ) {
	printf STDOUT "[Connect from DNS $HandyClientHostName <-> IP $HandyClientIPAdress] - Rejected by IPBlocker\n";
...
sub ResultFunction(){
	...
	if ( $QueryNumber eq 'NORESULTS' ) {
		...
		if ( length($SendString) != 0 ) {
			...
			print " Failure\n";
			$IO->writeSocket($HandyClientSocket, "FC 105" . CRLF); # FC 105 senden - query invalid
			return 0;
		}; # if ( length($SendString) != 0 ) {}

	}; # if ( $QueryNumber eq 'NORESULTS' ) {}
	
	my $DownloadHashRef			= $PARSER->ResultCommandParser( $MutellaSocket, $QueryNumber, $FileTypeHashRef );
