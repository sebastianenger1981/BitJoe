...
function writeStatusFieldHtmlPageAllServers( $ParameterAction ){
	...
	foreach ( $PhexRemoteControlServer as $Server ) { 
		...
		if (!$fp) {
			...
			$RequestString	= "$ParameterAction##$SecurityToken\r\n";
			...
			} elseif (strcasecmp("freemem", $ParameterAction) == 0) {
				
				list( $freemem,$freeswap ) = explode("#", $ReturnString );
				$ResultString	= "<br>Freier Hauptspeicher: <b>$freemem MByte</b> <br> Freier Swap:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$freeswap MByte";
...
function writeStatusFieldHtmlPage( $ParameterActionServer, $ParameterAction){
	...
	if (!$fp) {
		...
		echo "<b><h1 color=\"red\">$ParameterActionServer -> $errstr ($errno) - SERVER DOWN!</h1></b><br />\n";
...
function writeStartHtmlPage(){
	...
	echo <<<END
		...
		<form method="get" action="index.php" target="_self">	
			<select name="a" style="width: 27mm">
				<option value="load">Load Status</option>
				<option value="freemem">Free Memory</option>
				<option value="memory">Phex Memory</option>
				<option value="uptime">Phex Uptime</option>
				<option value="status">Phex Status</option>
				<option value="restart">Phex Restart</option>
			</select>
			<select name="ip" style="width: 26mm">
