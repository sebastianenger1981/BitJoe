 ...
 * 
 *  --- SVN Information ---
 *  $Id: SearchResultsDataModel.java 4219 2008-07-02 09:06:13Z gregork $
 */
...
public class SearchResultsDataModel implements ISearchDataModel
{
    ...
    public SearchResultsDataModel( Search search, SearchFilterRules filterRules, ArrayList ipfilter, String suffix)
    {
        this.search = search;
        ...
        this.suffix = suffix;
        searchToDataModelMap.put( search, this );
        //++++++++++++++++++++++++++++++++++++++++++++++++++++
    }
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    public SearchTreeTableModel getSearchTreeTableModel(){
    ...
    public void setIpFilter(ArrayList ipfilter){
    	this.ipfilter = ipfilter;
    }
    private boolean isIpFiltered(String ip){
    	for(int i=0; i < ipfilter.size(); i++){
    		if(ip.startsWith((String)ipfilter.get(i))){
    			return true;
    		}	
    	}
    	return false;	
    ...
    private void addSearchResults( RemoteFile[] newSearchResults, boolean fireEvents )
    {
        ...
        synchronized( displayedSearchResults )
        {
            ...
            for ( int i = 0; i < newSearchResults.length; i++ )
            {
                ...
                {
                	if(!isIpFiltered(remoteFile.getHostAddress().toString())){
                		if(suffix.equals("*")){
                			addSearchResultForDisplay( remoteFile, fireEvents );
                		}else if((suffix).contains(remoteFile.getFileExt())){
                			addSearchResultForDisplay( remoteFile, fireEvents );
                		}	
                   }
                }
    ...
    public ArrayList getAllRemoteFiles(){
    	al.clear();
    	//SearchResultElement sre;
    	for(int i = 0; i<displayedSearchResults.size(); i++){
    		rf = ((SearchResultElement)displayedSearchResults.get(i)).getRemoteFiles();
    		for(int j = 0; j < rf.length; j++ ){
    			al.add(rf[j]);
    		}	
    		//al.addAll(((SearchResultElement)displayedSearchResults.get(i)).getAlRemoteFiles());
    		//al.addAll(((SearchResultElement)displayedSearchResults.get(i)).getRemoteFiles());
    	}
    	return al;	
    ...
    private void processFilterRules( RemoteFile[] remoteFiles )
    {
        ...
        {
            quickFilterRule.process(search, remoteFiles, Servent.getInstance() );
        }
        for ( Rule rule : searchFilterRuleSet )
        {
            rule.process(search, remoteFiles, Servent.getInstance() );
        }
    }
...
//        }
    ...
    public static SearchResultsDataModel registerNewSearch( Search search, SearchFilterRules filterRules,ArrayList ipfilter, String suffix )
    {
        SearchResultsDataModel dataModel = new SearchResultsDataModel( search, filterRules, ipfilter, suffix);
