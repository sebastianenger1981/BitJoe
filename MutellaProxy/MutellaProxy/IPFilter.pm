...
sub ReloadFilter(){
	...
	if ( $Reload == 1 ) {
		...
		open(RELOAD, "<$REALOADFILE") or sub { warn "MutellaProxy::IPFilter->ReloadFilter(): Blocking not possible: IO ERROR: $!\n"; return -1; };
			flock (RELOAD, 2);
			$Reload = <RELOAD>;
		close RELOAD;
	...
	if ( $Reload == 0 ) {
		...
		# öffne ipfilter.dat
		open(FILTER, "<$IPFILTER") or sub { warn "MutellaProxy::IPFilter->ReloadFilter(): Blocking not possible: IO ERROR: $!\n"; return -1; };
			flock (FILTER, 2);
		...
		open(TMPFILTER, "<$TMPFILTER") or sub { warn "MutellaProxy::IPFilter->ReloadFilter(): Blocking not possible: IO ERROR: $!\n"; return -1; };
			flock (TMPFILTER, 2);
...
sub GenerateIPAdressesFromRange(){
	...
	if ( $Flag eq 'BOTH' ) {
		...
		open(WRITE, "+>>$TMPFILTER") or sub { warn "MutellaProxy::IPFilter->GenerateIPAdressesFromRange(): Blocking not possible: IO ERROR: $!\n"; return -1; };
			flock (WRITE, 2);
