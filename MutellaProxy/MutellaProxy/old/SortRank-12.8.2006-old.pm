		...
		if ( $AllowedFileTypeFlag > 0 ) {
			...
			if ( ($matched == 1) || (length( $FileName ) != 0) || ( $OnlyFileName =~ /$FileName/ ) || ( lc($OnlyFileName) eq lc($FileName) ) ){ 	
				...
				if ( ( $PeerHost =~ /(\d{0,255})+(\.)(\d{0,255})+(\.)(\d{0,255})+(\.)(\d{0,255})+(\:)(\d{1,})/ ) && ( $FileIndex =~ /(\d)/ ) ) {
					...
					# $SortedHashRefCounter = keys %{$SortedHashRef};
				...
				if ( $NextAllowedFileTypeFlag > 0 ) {
					...
					if ( $SHA1 eq $SHA1Next || $SHA1 == $SHA1Next ) {
						...
						if ( $BadCount == 0 ) {
							...
							eval {
								$NextMatched = amatch($NextOnlyFileName, $ApproxMatchingQuantifier, $FileNameNext);
							};

							if ( ($NextMatched == 1) || (length( $FileNameNext ) != 0) || ( $NextOnlyFileName =~ /$FileNameNext/ ) || ( lc($NextOnlyFileName) eq lc($FileNameNext) ) ){ 	
