...
# Hinweis: bei einer neueren Version von Sort::Array kann es sein, das beim sortieren nicht ein zusätzliches zeichen angehängt wird,
...
sub SortRank(){
	...
	while( my ($key, $value) = each( %{$ResultHashRef} ) ) {
		...
		if ( (length($FileName) > 0) && (length($FileName) <= $MaxFileLength) && (length($Speed) > 0) && (length($SHA1) == 32) && (length($Size) > 0) && (length($FileIndex) > 0) && ($PeerHost =~ /^((25[0-4]|(2[0-4]|1[0-9]|[1-9]?)[0-9]\.){3}(25[0-4]|(2[0-4]|1[0-9]|[1-9]?)[0-9]))+(\:)([0-9]{1,})$/ && length($PeerHost) >= 9) ) {	# 9 ist minimale gültige ip:port combi
			...
			# FILETYPBESTIMMUNG SCHLÄGT FEHL: BEI TEST.JPEG.JPG
			# buggy: my ( $OnlyFileName , $OnlyFileTyp ) = split(/[(\w)|(\d)]+(\.([^.]+?)$)/i, %{$ResultHashRef}->{ $key }->{ 'FN' } );

			my ( $OnlyFileName , $OnlyFileTyp ) = split(/(\.([^.]+?)$)/i, %{$ResultHashRef}->{ $key }->{ 'FN' } );
			
			if ( ( lc($OnlyFileTyp) eq lc $FileTypeHashRef->{1} ) || ( lc($OnlyFileTyp) =~ /$FileTypeHashRef->{1}/ ) ) {
				...
				while( my ($NextKey, $NextValue) = each( %{$ResultHashRef} ) ) {
					...
					if ( $SHA1 eq $SHA1Next ) {
						...
						for ( my $i=0; $i<=$#IsDoublePeer; $i++ ) {
							...
							# neu:	if ( $peer eq $PeerHostNext || $peer cmp $PeerHostNext || $peer == $PeerHostNext ) {
