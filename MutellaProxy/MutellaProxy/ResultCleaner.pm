...
sub DeleteEntryRange(){
	...
	for ( my $i=0; $i<=$DeleteUntil; $i++ ) {
		MutellaProxy::Mutella->del( $MutellaSocket, $i );	
	}; # for ( my $i=0; $i<=$DeleteUntil; $i++ ) {}
...
sub DeleteOldEntries(){
	...
	my $MutellaSocket	= MutellaProxy::IO->CreateMutellaSocket();
	my $ArrayRef		= MutellaProxy::IO->ReadFileIntoArray( $FILERESULTSTORE );
	
	srand();
	my $Random	= int(rand(10000))+1;
	
	open(WH, "+>>$STOREOLDRESULTS") or sub { warn "MutellaProxy::ResultCleaner->DeleteOldEntries(): IO ERROR: $!\n"; return -1; };
	flock (WH, 2);
	...
	open(CLEANER, "+>>$FILERESULTSTORE") or sub { warn "MutellaProxy::ResultCleaner->DeleteOldEntries(): IO ERROR: $!\n"; return -1; };
	flock (CLEANER, 2);
		...
		foreach my $entry ( @{$ArrayRef} ) {
			...
			if ( $CurTime >= ($Time+$CacheValidForSec) ) {
				...
				my $QUERYID = MutellaProxy::Mutella->getResultID( $MutellaSocket, $Search );
				if ( $QUERYID eq 'NORESULTS' ) {
				} else {
					
					MutellaProxy::Mutella->del( $MutellaSocket, $QUERYID );
					my $QID = MutellaProxy::Mutella->getResultID( $MutellaSocket, $Search );
					if ( $QID ne 'NORESULTS' ) {
						MutellaProxy::Mutella->del( $MutellaSocket, $QID );
					};
