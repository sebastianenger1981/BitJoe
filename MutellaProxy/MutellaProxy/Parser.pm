...
sub CheckSharingRatio(){
	...
	while ( length($list) == 0 ) {
		...
		$list = MutellaProxy::IO->readSocket( $MutellaSocket );
	...
	for ( my $a=0; $a<=$#BadIPs; $a++ ) {
		...
		MutellaProxy::IO->writeSocket( $MutellaSocket, CLOSE_COMMAND . "$Peer\n" );	
...
sub ResultCommandParser(){
	...
	while ( length($list) == 0 ) {
		MutellaProxy::IO->writeSocket( $MutellaSocket, RESULTS_COMMAND . $ResultNumber . " \n" );
		$list = MutellaProxy::IO->readSocket( $MutellaSocket );
		select( undef, undef, undef, 0.2 );
	};
	...
	for ( my $i=0; $i<=$#content; $i++ ) {
		...
		if ( $content[$i] =~ /RSID:+[\s{1}]+[\d{1,}]+[\)]/i ){ 
			...
			my $StatusA = MutellaProxy::SpamFilter->SpamFilter( \$FILENAME, $SearchQuery );
			if ( $StatusA == -1 ) {
			...
			my $StatusB = MutellaProxy::Filter->SizeFilter( $FileTypeHashRef->{ 1 }, $SIZE );
			if ( $StatusB == -1 ) {
			...
			my $StatusC = MutellaProxy::IPFilter->SimpleFilter( $PEER );
			if ( $StatusC == -1 ) {
				#	$DownloadHashRef->{ $ResultCount }->{ 'PH' } = '';
				#	delete $DownloadHashRef->{ $ResultCount };
				#	undef $DownloadHashRef->{ $ResultCount };
...
sub ListCommandParser(){
	...
	while ( length($list) == 0 ) {
		MutellaProxy::IO->writeSocket( $MutellaSocket, LIST_COMMAND );
		$list = MutellaProxy::IO->readSocket( $MutellaSocket );
		select( undef, undef, undef, 0.2 );
	};
