...
sub decompress_string_zlib(){
	...
	my $RandString	= time();	# in MutellaProxy::IO->WriteFile wird schon ein randomized Filename generiert
	my $file		= MutellaProxy::IO->WriteFile("$temppath/gziptostring.$RandString.temp.txt", $string );
	...
	my $ContentScalar = MutellaProxy::IO->ReadFileIntoScalar( $gzfile );
	unlink $gzfile;
