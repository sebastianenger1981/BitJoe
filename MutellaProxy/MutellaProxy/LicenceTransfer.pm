...
sub EncryptLicenceForTransfer(){
	...
	#	'TO2'		=> 'torsten.morgenroth@gmail.com',
		'SUBJECT'	=> 'MD5 - Werte verschlüsselten Lizenzdaten vom ' . MutellaProxy::Time->MySQLDateTime(),
		'BODY'		=> '',
	};

	my $ContentScalarRef1		= MutellaProxy::IO->ReadFileIntoScalar( $FileCrypto );
	my $Crypted					= MutellaProxy::CryptoLibrary->Encrypt( $CRYPTOKEY, $ContentScalarRef1 );
	my $MD5ofCryptedContent1	= MutellaProxy::CheckSum->MD5ToHEX($Crypted);
	
	MutellaProxy::IO->WriteFile( $FileCryptoEncrypted, \$Crypted );

	my $ContentScalarRef2		= MutellaProxy::IO->ReadFileIntoScalar( $FileLicence ); 
	my $Crypted					= MutellaProxy::CryptoLibrary->Encrypt( $CRYPTOKEY, $ContentScalarRef2 );
	my $MD5ofCryptedContent2	= MutellaProxy::CheckSum->MD5ToHEX($Crypted);
	
	MutellaProxy::IO->WriteFile( $FileLicenceEncrypted, \$Crypted );
					
	$MailConfigHashRef->{'BODY'} = "MD5 Werte der CryptoDaten $FileCryptoEncrypted: $MD5ofCryptedContent1\n";

	MutellaProxy::Mail->SendMail( $MailConfigHashRef );
...
sub DecryptLicence(){
	...
	my $ContentScalarRef	= MutellaProxy::IO->ReadFileIntoScalar( $FileCryptoEncrypted ); 
	my $PlainText			= MutellaProxy::CryptoLibrary->Decrypt( $CRYPTOKEY, $ContentScalarRef );
	
	MutellaProxy::IO->WriteFile( $FileCryptoDecrypted, \$PlainText );	

	my $ContentScalarRef	= MutellaProxy::IO->ReadFileIntoScalar( $FileLicenceEncrypted ); 
	my $PlainText			= MutellaProxy::CryptoLibrary->Decrypt( $CRYPTOKEY, $ContentScalarRef );
	
	MutellaProxy::IO->WriteFile( $FileLicenceDecrypted, \$PlainText );	
...
sub CheckIfEncryptedFileExists(){
	...
	if ( -e $FileCryptoEncrypted ) {
		...
		print "$0 -> MutellaProxy::LicenceTransfer->CheckIfEncryptedFileExists(): File '$FileCryptoEncrypted' does not exist -> Exit now!\n";
		return -1;
	};
...
sub SendCheckSumOfFiles(){
	...
	my $ContentScalarRef	= MutellaProxy::IO->ReadFileIntoScalar( $FileCryptoEncrypted ); 
	my $CheckSum			= MutellaProxy::CheckSum->MD5ToHEX( $ContentScalarRef );
...
sub InstallLicenceToDataBase(){
	...
	my $ContentArrayRef1 = MutellaProxy::IO->ReadFileIntoArray( $FileCryptoDecrypted );
	my $ContentArrayRef2 = MutellaProxy::IO->ReadFileIntoArray( $FileLicenceDecrypted );
...
sub TestInstalledLicence(){
	...
	if ( keys(%{$ResultHashRef}) == 8 && keys(%{$ResultHashRef2}) == 8 ) {
		my $MailConfigHashRef = {
			'TO1'		=> 'thecerial@gmail.com',
			'SUBJECT'	=> 'Lizenzdaten korrekt installiert ' . MutellaProxy::Time->MySQLDateTime(),
			'BODY'		=> '',
		};
		MutellaProxy::Mail->SendMail( $MailConfigHashRef );
		return 1;
		...
		my $MailConfigHashRef = {
			'TO1'		=> 'thecerial@gmail.com',
			'SUBJECT'	=> 'Lizenzdaten nicht richtig installiert ' . MutellaProxy::Time->MySQLDateTime(),
			'BODY'		=> '',
		};
		MutellaProxy::Mail->SendMail( $MailConfigHashRef );
		return 0;
