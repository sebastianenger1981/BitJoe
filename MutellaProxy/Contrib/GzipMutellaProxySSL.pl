...
##############
my $MutellaSocket	= MutellaProxy::IOSSL->CreateMutellaSocket( $TIMEOUT );
my $ProxySocket		= MutellaProxy::IOSSL->CreateProxySocket( $port );
...
sub FindFunction(){
	...
	#	$IO->WriteFile( "$FILE/$ClientID", "$IOReadFromClient[5] .$FileTypeHashRef->{1}" );	# für phexproxy
