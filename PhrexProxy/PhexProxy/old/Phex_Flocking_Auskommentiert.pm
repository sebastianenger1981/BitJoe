...
my $PhexSleepTimeAfterCommand		= 0.00001;	# Warten in sekunden zwischen den einzelnen Commandos
...
sub find(){
	...
	#	### Flocking Algorithmus: Start
	#	open(FLOCK,">>$FLOCKINGFILE");
	#	flock(FLOCK, LOCK_EX);
		
			# local $SIG{'ALRM'} = sub { warn "Paris ' $PhexServerIP' not serving results in 3 seconds \n" && last ALARMLABEL; };
			# local $SIG{'ALRM'} = return -1;
			# alarm $AlarmTimeout;
			...
			# print FIND "[$PhexServerIP] find $ClientID 0 $FileSizeLimitInBytes Any $Token $SearchQuery at " . time() . " \n";
			close($PhexSocket);
	#		select (undef, undef, undef, $PhexSleepTimeAfterCommand );		
	#		
	#	flock(FLOCK, LOCK_UN);
	#	close FLOCK;
	#	### Flocking Algorithmus: End
...
#		select (undef, undef, undef, $PhexSleepTimeAfterCommand );	
#
#		# Sende Del Befehl nach erfolgreichem abholen der ergebnisse
#		if ( length($Results) > 4 ) {
#			select (undef, undef, undef, $PhexSleepTimeAfterCommand );
#			
