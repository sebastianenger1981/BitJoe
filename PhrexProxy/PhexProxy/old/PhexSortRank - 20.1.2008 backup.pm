...
############ debug: ende
		...
		if ( length($_) > 20 ) {
			...
			#		if ( $StatusA == 1 && $StatusB == 1 ) {
					if ( $StatusB == 1 ) {
						...
						#	$SortedHashRef->{ $SortedHashRefCounter }->{ 'MD5' } = PhexProxy::CheckSum->MD5ToHEX($MD5ValuePack);	# md5 wert erstellen
	...
	print "I have $SortHashRefCount PreSorted Results\n";
	...
	foreach my $Key ( sort { $SortedHashRef->{$b}->{'SH'} <=> $SortedHashRef->{$a}->{'SH'} } keys %{$SortedHashRef} ) {
		...
		my $MD5Value		= %{$SortedHashRef}->{ $Key }->{ 'MD5' };

	#	next if ( length($FileName) <= 0 );
	#	next if ( length($Speed) <= 0 );
	#	next if ( length($SHA1) <= 0 );
	#	next if ( length($Size) < 32 );
	#	next if ( length($PeerHost) <= 0 );

	#	my @IsDoublePeer	= ();
	#	my @IsDoublePeer	= split("\t\t", $PeerHost);
	#	my $IsDoublePeer;
	#	print "($Key) 1.foreach - $FileName \n";
		...
		foreach my $NextKey ( sort { $SortedHashRef->{$b}->{'SH'} <=> $SortedHashRef->{$a}->{'SH'} } keys %{$SortedHashRef} ) {
			...
			my $MD5ValueNext	= %{$SortedHashRef}->{ $NextKey }->{ 'MD5' };

		#	print "\t($NextKey) 2.foreach - $FileNameNext \n";

		#	print "FILE: ". length($FileNameNext). "\n";
		#	print "SIZE: ". length($SizeNext). "\n";
		#	print "SPEED: ". length($SpeedNext). "\n";
		#	print "SHA1: ". length($SHA1Next). "\n";
		#	print "PEER: ". length($PeerHostNext) . "\n";
			
		#	next if ( $MD5Value eq $MD5ValueNext );		# ignoriere genau gleiche Einträge - sie würden sonst nur zu doppelten quellen führen
		...
		#	next if ( length($PeerHostNext) <= 0 );
			...
			} else { #if ( ($SHA1 ne $SHA1Next) ) { 
			
				print "DIFFERENT: $SHA1 --- $SHA1Next\n";
			#	my $SpeedPoints = GetSpeedPoints($SpeedNext);	
			#
			#	$SortedHashRefResults->{ $SHA1 }->{ 'PH' } = $PeerHostNext .';'. $FileIndexNext .';'. $FileNameNext . ';' . $SpeedPoints . "\t\t";		
			#	$SortedHashRefResults->{ $SHA1 }->{ 'RK' } = $SpeedPoints; 
			#	$SortedHashRefResults->{ $SHA1 }->{ 'SZ' } = $Size;
			#	$SortedHashRefResults->{ $SHA1 }->{ 'SH' } = $SHA1Next;
			#
			#	# hier dann den aktuellen Next Eintrag löschen
			#	$SortedHashRef->{ $NextKey } = "";
			#	delete $SortedHashRef->{ $NextKey };
